<script setup>
  ...
  const checkStatus = async () => {
    try {
      let exchange = {};
      ({data: {exchange}} = await httpClient.get('/exchange', {
        searchParams: {
...
<template>
  <div class="-mt-72 bg-white z-10 mx-auto p-10 rounded-md max-w-3xl px-16 lg:px-24 relative">
      ...
      <p class="mb-6" v-html="translations[defaultLanguage].app_install_explain"></p>
      ...
      <p class="text-center mb-2" v-html="translations[defaultLanguage].qr_explain"></p>
      <p class="text-center">
