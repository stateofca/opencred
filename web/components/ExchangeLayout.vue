...
import DCApiView from "./DCApiView.vue";
...
const switchView = () => {
    state.currentUXMethodIndex =
        (state.currentUXMethodIndex + 1) %
...
onBeforeMount(async () => {
    ...
    try {
        ...
        // Set initial view based on workflow type
        if (resp.data.rp?.workflow?.type === "dc-api") {
            const dcApiIndex =
                config.options.exchangeProtocols.indexOf("dc-api");
            if (dcApiIndex !== -1) {
                state.currentUXMethodIndex = dcApiIndex;
            }
        } else if (resp.data.rp?.workflow?.type === "vc-api") {
            // For VC-API workflows, prefer openid4vp if available
            const openid4vpIndex =
                config.options.exchangeProtocols.indexOf("openid4vp");
            if (openid4vpIndex !== -1) {
    ...
    try {
        ...
        if (exchange.state === "complete") {
            ...
            handleError({
                ...
                subtitle: translate("exchangeErrorSubtitle"),
                message:
                    Object.values(exchange.variables.results ?? {})
...
<template>
    <div class="flex flex-col min-h-screen">
        <header :style="{ background: context.rp.brand.header }">
            ...
            >
                <a
                    ...
                    :href="
                        context.rp.primaryLogo?.href ?? context.rp.primaryLink
                    "
                ...
                >
                    <img
                        ...
                        :src="
                            typeof context.rp.primaryLogo === 'string'
                                ? context.rp.primaryLogo
                                : context.rp.primaryLogo?.id
                        "
                ...
                <a
                    ...
                    :href="
                        context.rp.secondaryLogo?.href ??
                        context.rp.secondaryLink
                    "
                    class="flex items-center gap-3"
                >
                    <img
                        ...
                        :src="
                            typeof context.rp.secondaryLogo === 'string'
                                ? context.rp.secondaryLogo
                                : context.rp.secondaryLogo?.id
                        "
                ...
                >
                    <q-btn-dropdown
                        v-if="
                            availableLocales.length > 1 && useNativeTranslations
                        "
                        flat
                        no-caps
                        text-color="white"
                    >
                        <template #label>
                            <div
                                class="row items-center no-wrap gap-2 text-white"
                            >
        ...
        <main class="relative flex-grow">
            <div v-if="context.rp.homeLink" class="bg-white w-full text-center">
                <h2 class="font-bold">
            ...
            <div
                ...
                :style="{
                    'background-image': `url(${context.rp.backgroundImage})`,
                }"
            >
            ...
            <CHAPIView
                v-else-if="
                    config.options.exchangeProtocols[
                        state.currentUXMethodIndex
                    ] === 'chapi'
                "
                :chapi-enabled="true"
            ...
            <DCApiView
                ...
                @override-active="state.activeOverride = true"
                @replace-exchange="replaceExchange"
                @reset-exchange="handleResetExchange"
            />
        ...
        <footer class="text-left p-3" v-html="$t('copyright')" />
    </div>
</template>
