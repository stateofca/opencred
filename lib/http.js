...
import {didConfigurationDocument, didWebDocument} from './didWeb.js';
import {
  exchangeCodeForToken,
  jwksEndpoint,
  OidcValidationMiddleware,
  openIdConfiguration,
} from './oidc.js';
import {getExchangeStatus, initiateExchange} from './api.js';
...
import {DCApiWorkflowService} from './workflows/dc-api-workflow.js';
import {EntraVerifiedIdWorkflowService} from './workflows/entra-verified-id-workflow.js';
import {NativeWorkflowService} from './workflows/native-workflow.js';
import {newExchangeContext} from './workflows/common.js';
import ResolveClientMiddleware, {
  attachClientByBody,
...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.get(routes.config, (req, res) => {
    ...
    const {
      ...
      // bedrock config!
      reCaptcha: {pages, version, siteKey},
   ...
   *         description: Internal server error.
   */
  // Token endpoint with body-based client authentication
  app.post(routes.token, attachClientByBody, exchangeCodeForToken);
