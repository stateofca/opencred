...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  const continuationContextHandler = async (req, res) => {
    ...
    const formattedExchange = await workflowService.formatExchange({
      exchange,
      workflow
    });
   ...
   *               continuationUrl:
   *                type: string
   *                description: URL to redirect the user to for completing
   *                  the verification in a new window. Includes an
   *                  exchange_token for authentication.
   *      "400":
   *        description: Request is malformed.
   *        content:
   *          application/json:
