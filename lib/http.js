...
const routes = {
  ...
  dcApiEvent: '/workflows/:workflowId/exchanges/:exchangeId/dc-api-event',
...
const DC_API_EVENT_TYPES = new Set(['cancelled', 'error', 'timeout']);

/**
 * Logs a client-reported DC API outcome (`navigator.credentials.get`
 * rejection, or a client-side timeout waiting on it) that never reaches the
 * server through any other route. Best-effort telemetry only; does not
 * touch exchange state or run verification.
 *
 * @param {object} req - Express request object; `req.exchange` and
 *   `req.workflow` are populated by `getExchangeMiddleware`.
 * @param {object} res - Express response object.
 */
export function dcApiEventMiddleware(req, res) {
  const {type, profile, errorName, timeoutMs} = req.body ?? {};
  if(!DC_API_EVENT_TYPES.has(type)) {
...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  */
  ...
  // POST /workflows/:workflowId/exchanges/:exchangeId/dc-api-event
  app.post(routes.dcApiEvent,
    attachClientByWorkflowId,
    baseWorkflowService.getExchangeMiddleware.bind(baseWorkflowService),
    getAuthFunction({basic: false, bearer: true, body: false}),
    dcApiEventMiddleware
  );
