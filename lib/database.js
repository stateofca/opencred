...
export const saveRootCertificates = async () => {
  const certs = config.opencred.caStore;
  if(!certs) {
    return;
  }
...
bedrock.events.on('bedrock-mongodb.ready', async function() {
  ...
  */
  ...
  await database.collections.Exchanges.createIndex({accessToken: 1});
  logger.info('Ensured exchange accessToken index exists');
