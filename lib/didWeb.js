...
export const didWebDocument = async (req, res) => {
  ...
  if(authzReqKey) {
    ...
    const publicKeyJwk = await exportJWK(key);
    // Use id from config if available, otherwise compute JWK thumbprint
    const fragment = authzReqKey.id ?? await calculateJwkThumbprint(
      publicKeyJwk);
    const vmId = `${domainToDidWeb(baseUri)}#${fragment}`;
