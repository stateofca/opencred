...
const createExchangeHelper = async (
  rp, oidcState = '', bodyAndQueryVariables = null
) => {
  ...
  const variables = {};
  for(const v of workflow.params) {
    if(v in bodyAndQueryVariables) {
      variables[v] = bodyAndQueryVariables[v];
    }
  }
  ...
  return {id: requestId, vcapi, OID4VP, accessToken, workflowId, oidc, rp};
};
...
const createExchange = async (req, res, next) => {
  ...
  try {
    req.exchange = await createExchangeHelper(
      rp,
      req.query?.state ?? req.body?.oidcState,
      {...req.query, ...req.body}
    );
  } catch(error) {
    logger.error(error);
...
export default function(app) {
  ...
  app.get('/context/verification', createExchange);
