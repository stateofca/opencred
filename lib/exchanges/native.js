...
export const createExchange = async (domain, workflow, oidcState = '') => {
  ...
  const searchParams = new URLSearchParams({
    client_id: domainToDidWeb(config.opencred.domain),
...
export const getExchange = async (id, {others, allowExpired} = {
  ...
  const exchange = await database.collections.Exchanges.findOne({id, ...others},
    {projection: {_id: 0}});
  if(!exchange) {
    return null;
  }
...
export const createNativeExchange = async (req, res, next) => {
  ...
  req.exchange = await createExchange(
    config.opencred.domain, rp.workflow, req.query.state
  );
  next();
};
    ...
    for(const descriptor of presentation_definition.input_descriptors) {
      ...
      if(!submitted) {
        ...
        const vpResult = await verifyUtils.verifyPresentationJWT(vp_token, {
          audience: domainToDidWeb(config.opencred.domain)
        });
...
export default function(app) {
  ...
  app.get('/context/login', createNativeExchange);
  ...
  app.get('/workflows/:workflowId/exchanges/:exchangeId/openid/client/' +
    ...
    try {
      ...
      vpr.domain = `${config.opencred.domain}${
      ...
      const jwt = await new SignJWT(authorizationRequest)
        .setProtectedHeader({
          alg: key.type,
          kid: `${domainToDidWeb(config.opencred.domain)}#${key.id}`,
          typ: 'JWT'
        })
        .setIssuedAt()
