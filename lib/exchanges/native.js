...
export const createExchange = async (
  workflow, oidcState = '', bodyAndQueryVariables = null
) => {
  ...
  const possibleParams = [...workflow.params, 'redirectUrl'];
  const variables = {};
  for(const v of Object.keys(bodyAndQueryVariables)) {
    if(possibleParams.indexOf(v) >= 0) {
...
export default function(app) {
  ...
  app.get('/context/verification', getOrCreateNativeExchange);
