...
export const getExchange = async ({params, allowExpired} = {
  ...
  const exchange = await database.collections.Exchanges.findOne(params,
    {projection: {_id: 0}});
  if(!exchange) {
    return null;
  }
...
export const createExchange = async (
  workflow, oidcState = '', redirectUri = null
) => {
  ...
  const variables = redirectUri ? {redirectUri} : {};
  const exchange = {
    id,
    workflowId,
    sequence: 0,
...
export const createNativeExchange = async (req, res, next) => {
  ...
  req.exchange = await createExchange(
    rp.workflow, req.query.state, req.body?.redirectUri ?? null
  );
  next();
};

export const getOrCreateNativeExchange = async (req, res, next) => {
  if(req.cookies.exchangeId && req.cookies.accessToken) {
    const exchange = await getExchange({
      params: {
        id: req.cookies.exchangeId,
    ...
    await createNativeExchange(req, res, next);
  }
};
  ...
  app.post('/workflows/:workflowId/exchanges/:exchangeId/openid/client/' +
    ...
    try {
      ...
      if(verified) {
        ...
        if(exchange.variables.redirectUri) {
          res.send({redirect_uri: exchange.variables.redirectUri});
        } else {
          res.sendStatus(204);
        }
        return;
