...
import {findWorkflowByCredentials} from './resolveClient.js';
import {verifyExchangeResultToken} from './workflows/common.js';
  ...
  const ensureAuth = async (req, res, next) => {
    ...
    if(body && !req.workflow) {
      ...
      if(body_id && body_secret) {
        req.workflow = findWorkflowByCredentials(
          config.opencred.workflows, body_id, body_secret
        );
        if(req.workflow) {
          next();
          return;
    ...
    if(basic && !req.workflow && parts.length > 0) {
      ...
      req.workflow = findWorkflowByCredentials(
        config.opencred.workflows, clientId, clientSecret
      );
      if(req.workflow) {
        next();
        return;
    ...
    if(!body && parts.length !== 2) {
      ...
      const val = Buffer.from(parts[1], 'base64').toString('utf-8');
      const authValueParts = val.split(':');
      const usernameMatchesClient = authValueParts[0] === clientId ||
        authValueParts[0] === req.workflow?.workflowId;
      if(
        authValueParts.length !== 2 ||
        !usernameMatchesClient ||
      ...
      let exchange = await baseWorkflowService.getExchange({
      ...
      if(!exchange) {
        ...
        if(looksLikeJwt) {
          try {
            const payload = await verifyExchangeResultToken(parts[1]);
            const workflowMismatch = req.params.workflowId &&
              payload.workflowId !== req.params.workflowId;
            if(workflowMismatch) {
              res.status(404).send({message: 'Exchange not found'});
              return;
            }
            exchange = await baseWorkflowService.getExchange({
            ...
            if(exchange.workflowId !== payload.workflowId) {
              res.status(404).send({message: 'Exchange not found'});
              return;
            }
            req.exchange = exchange;
            req.exchangeScope =
              payload.scope === 'exchange:partial' ? 'partial' : undefined;
          } catch {
