  ...
  if(config.opencred.reCaptcha.enable) {
    ...
    try {
      ...
      if(!reCaptchaResponse.data.success) {
        ...
        logger.error('reCAPTCHA error response:',
          {error: new Error(JSON.stringify(reCaptchaResponse.data, null, 2))});
        return res.status(400).send({
          ...defaultErrorResponse,
          message: errorMessage
        });
      } else {
        logger.debug('reCAPTCHA success response:', reCaptchaResponse.data);
      }
    } catch(error) {
      if(error) {
