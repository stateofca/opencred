...
export class EntraVerifiedIdWorkflowService extends BaseWorkflowService {
  getProtocols({exchange, workflow} = {}) {
    return {
      ...
      OID4VP: `${exchange.OID4VP}`,
      // No news yet that they've updated to 1.0
      'OID4VP-draft18': exchange.OID4VP
    };
  }

  async createWorkflowSpecificExchange(trustedVariables, untrustedVariables) {
    const {workflow} = trustedVariables;
    if(workflow?.type !== 'microsoft-entra-verified-id') {
      return;
    }

    const acceptedIssuers = workflow.trustedCredentialIssuers?.length > 0 ?
      workflow.trustedCredentialIssuers : undefined;
    ...
    const verificationPayload = {
      includeQRCode: false,
      includeReceipt: true,
      authority: workflow.verifierDid,
      registration: {
        clientName: workflow.verifierName
      },
      callback: {
      ...
      requestedCredentials: [
        {
          type: workflow.acceptedCredentialType,
          purpose: workflow.credentialVerificationPurpose ??
            'To check permission to access resources',
          acceptedIssuers,
          configuration: {
            validation: {
              allowRevoked: !!workflow.allowRevokedCredentials,
              validateLinkedDomain: !!workflow.validateLinkedDomain
            }
          }
        }
      ]
    ...
    let response;
    try {
      const msalClient = msalUtils.getMsalClient(workflow);
      response = await msalUtils.makeHttpPostRequest({
        msalClient,
        url: `${
          workflow.apiBaseUrl}/verifiableCredentials/createPresentationRequest`,
    ...
    ex.OID4VP = `openid4vp://?request_uri=${
      requestUri}&client_id=${workflow.verifierDid}`;
    return this.formatExchange({exchange: ex, workflow});
  }

  async verificationCallbackMiddleware(req, res) {
    ...
    if(!workflow) {
      const errorMessage = 'Could not find workflow configuration';
      logger.error(errorMessage, {error: new Error(errorMessage)});
    ...
    if(exchangeState === 'active') {
      // `entra` names the workflow that started the presentation; it is an
      // event-stream value, not a registered profile identifier.
      logUtils.presentationStart(workflow.clientId, exchange.id, ['entra']);
    }
