        ...
        if(isValidJson(vpToken)) {
          ...
          for(const vcToken of vcTokens) {
            ...
            if(!vcResult.verified) {
              ...
              if(
                config.opencred.caStore.length > 0
              ) {
                const certs = await extractCertsFromX5C(
                  vcResult.signer.publicKeyJwk
                );
                const enforceCert = shouldEnforceRpCertCheck(rp);

                if(certs) {
                  if(enforceCert) {
                    logger.warn(
                      'x5c claim provided but enforcement is disabled.' +
                      'Verifying the certificate chain anyway.'
                    );
                  }
