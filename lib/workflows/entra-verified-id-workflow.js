...
export class EntraVerifiedIdWorkflowService extends BaseWorkflowService {
  getProtocols(exchange, {rp} = {}) {
    if(!exchange || !rp || rp.type !== 'microsoft-entra-verified-id') {
      return {};
    }
  ...
  async createWorkflowSpecificExchange(trustedVariables, untrustedVariables) {
    ...
    if(rp?.type !== 'microsoft-entra-verified-id') {
      return;
    }
    ...
    const verificationPayload = {
      includeQRCode: false,
      includeReceipt: true,
      authority: rp.verifierDid,
      registration: {
        clientName: rp.verifierName
      },
      callback: {
      ...
      requestedCredentials: [
        {
          type: rp.acceptedCredentialType,
          purpose: rp.credentialVerificationPurpose ??
            'To check permission to access resources',
          acceptedIssuers: trustedCredentialIssuers,
          configuration: {
            validation: {
              allowRevoked: !!rp.allowRevokedCredentials,
              validateLinkedDomain: !!rp.validateLinkedDomain
            }
          }
        }
      ]
    ...
    try {
      ...
      response = await msalUtils.makeHttpPostRequest({
        msalClient,
        url: `${rp.apiBaseUrl}/verifiableCredentials/createPresentationRequest`,
    ...
    ex.OID4VP =
      `openid4vp://?request_uri=${requestUri}&client_id=${rp.verifierDid}`;
    return this.formatExchange(ex, {rp});
  }
