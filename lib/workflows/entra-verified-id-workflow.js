...
export class EntraVerifiedIdWorkflowService extends BaseWorkflowService {
  ...
  async verificationCallback(req, res) {
    ...
    const presentationStartEvent =
      presentationUtils.getPresentationStartEvent('unknown');
    const presentationSuccessEvent =
      presentationUtils.getPresentationSuccessEvent('unknown');
    const presentationErrorEvent =
      presentationUtils.getPresentationErrorEvent('unknown');
    if(!requestId) {
      const errorMessage = 'Missing required request parameter: "requestId"';
      logger.error(errorMessage, {error: new Error(errorMessage)});
      logger.info('Event', presentationErrorEvent);
      res.status(400).send(
    ...
    presentationStartEvent.relyingParty = rp.clientId;
    presentationSuccessEvent.relyingParty = rp.clientId;
    presentationErrorEvent.relyingParty = rp.clientId;
    ...
    if(verificationError) {
      ...
      const errorMessage =
        verificationError.message ?? 'Unknown Entra error';
    ...
    switch(verificationStatus) {
      ...
      case VerificationStatus.PresentationError:
        exchangeState = 'invalid';
        if(!verificationError) {
          const errorMessage = 'Unknown Entra verification error';
          errors.push(errorMessage);
          logger.error(errorMessage, {error: new Error(errorMessage)});
        }
        break;
    ...
    if(exchangeState === 'active') {
      logger.info('Event', presentationStartEvent);
    }
    if(exchangeState === 'complete') {
      logger.info('Event', presentationSuccessEvent);
