...
export class EntraVerifiedIdWorkflowService extends BaseWorkflowService {
  ...
  async verificationCallback(req, res) {
    ...
    if(!requestId) {
      ...
      logger.info('Event', logEvents.presentationError(
        'unknown', 'unknown', errorMessage));
      res.status(400).send(
    ...
    if(!rp) {
      ...
      logger.error(errorMessage, {error: new Error(errorMessage)});
      logger.info('Event', logEvents.presentationError(
        'unknown', exchange.id, errorMessage
      ));
      res.status(404).send({message: errorMessage});
      return;
    }
    ...
    if(!accessToken) {
      ...
      logger.error(errorMessage, {error: new Error(errorMessage)});
      logger.info('Event', logEvents.presentationError(
        rp.clientId, exchange.id, errorMessage));
    ...
    if(exchangeState === 'active') {
      logger.info('Event', logEvents.presentationStart(
        rp.clientId, exchange.id));
    }
    if(exchangeState === 'complete') {
      logger.info('Event', logEvents.presentationSuccess(
        rp.clientId, exchange.id));
