...
export class EntraVerifiedIdWorkflowService extends BaseWorkflowService {
  ...
  async verificationCallback(req, res) {
    ...
    if(!requestId) {
      ...
      logger.info('Event', logEvents.errorEvent('unknown'));
      res.status(400).send(
    ...
    if(!exchange) {
      logger.info('Event', logEvents.errorEvent(req.rp?.clientId));
    ...
    if(!accessToken) {
      ...
      logger.error(errorMessage, {error: new Error(errorMessage)});
      logger.info('Event', logEvents.errorEvent(rp.clientId));
    ...
    if(exchangeState === 'active') {
      logger.info('Event', logEvents.presentationStart(rp.clientId));
    }
    if(exchangeState === 'complete') {
      logger.info('Event', logEvents.presentationSuccess(rp.clientId));
