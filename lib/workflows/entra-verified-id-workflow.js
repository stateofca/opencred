...
export class EntraVerifiedIdWorkflowService extends BaseWorkflowService {
  ...
  async verificationCallback(req, res) {
    ...
    if(!requestId) {
      res.status(400).send(
        {message: 'Missing required request parameter: "requestId"'});
      return;
    }
    ...
    if(!exchange) {
      res.status(404).send({message: 'Could not find exchange'});
      return;
    }
    ...
    if(!rp) {
      res.status(404).send({message: 'Could not find relying party'});
      return;
    }
