...
export class BaseWorkflowService {
  ...
  async resetExchange(req, res) {
    ...
    if(exchange?.state !== 'invalid') {
      res.status(400).send({message: 'Exchange cannot be found or reset.'});
      return;
    }
  ...
  async getExchange(
    ...
    // Necessary for removing secret access token
    // from frontend for Entra clients
    delete exchange.apiAccessToken;
    if(!exchange || !rp) {
      return null;
    }
