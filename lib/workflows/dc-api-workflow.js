...
export class DCApiWorkflowService extends BaseWorkflowService {
    ...
    const dcApi = await DcApi.new(
      ...
      // DC API Session
      {
        newSession: async (sessionId, session) => {
          // NOTE: This callback is handled within the `createWorkflowSpecificExchange`
          // when `create_new_session` is called.
          // The session is stored directly within the exchange.
        },
        getSession: async (id, clientSecret) => {
          const exchange = await database.collections.Exchanges.findOne(
            { "variables.dcApiSession.id": id },
            { projection: { _id: 0, variables: 1 } },
          );
          return exchange?.variables?.dcApiSession || null;
        ...
        updateSession: async (sessionId, session) => {
          // Use the exchangeId stored IN the session object for efficient lookup
          if (session.exchangeId) {
            await database.collections.Exchanges.updateOne(
              { id: session.exchangeId },
              { $set: { "variables.dcApiSession": session } },
            );
          } else {
            // Fallback: search by session.id
            await database.collections.Exchanges.updateOne(
              { "variables.dcApiSession.id": sessionId },
              { $set: { "variables.dcApiSession": session } },
            );
          }
  ...
  async createWorkflowSpecificExchange(trustedVariables, untrustedVariables) {
    ...
    // // Set the exchangeId on the dcApiSession
    // dcApiSession.exchangeId = ex.id;
    // dcApiSession.workflowId = ex.workflowId;
