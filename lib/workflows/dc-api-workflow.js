...
export class DCApiWorkflowService extends BaseWorkflowService {
  ...
  async initDcApi() {
    ...
    const dcApi = await DcApi.new(
      ...
      this.createOID4VPSessionStore(),
      this.createDCAPISessionStore(),
    );
  ...
  createOID4VPSessionStore() {
    return new JsOid4VpSessionStore({
      async initiate(session) {
        try {
          await database.collections.Exchanges.updateOne(
            {
              'variables.dcApiSession.session_creation_response.id': session.id,
            },
            {$set: {'variables.oid4vpSession': session}},
          );
        } catch(e) {
