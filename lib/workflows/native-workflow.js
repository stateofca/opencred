...
export class NativeWorkflowService extends BaseWorkflowService {
  ...
  async authorizationRequest(req, res) {
    ...
    const presentationStartEvent =
      presentationUtils.getPresentationStartEvent(rp?.clientId ?? 'unknown');
    const presentationErrorEvent =
      presentationUtils.getPresentationErrorEvent(rp?.clientId ?? 'unknown');
  ...
  async authorizationResponse(req, res) {
    ...
    const presentationSuccessEvent =
      presentationUtils.getPresentationSuccessEvent(rp?.clientId ?? 'unknown');
    const presentationErrorEvent =
      presentationUtils.getPresentationErrorEvent(rp?.clientId ?? 'unknown');
    if(!exchange) {
      logger.info('Event', presentationErrorEvent);
