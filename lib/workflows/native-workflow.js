...
import {
  buildExchangeResultToken, handleVerifiedPresentation
} from './common.js';
import {
  clientIdForProfile,
...
import {identifyProfile} from './common/identify-profile.js';
...
export class NativeWorkflowService extends BaseWorkflowService {
  ...
  async formatExchange({exchange, includeQR = false, workflow} = {}) {
    return super.formatExchange({exchange, includeQR, workflow});
  }
   ...
   */
  async processCallback({workflow, updatedExchange, userAgent}) {
  ...
  async authorizationRequestMiddleware(req, res, next) {
    ...
    try {
      ...
      const {profile, responseMode, clientIdScheme, signed} = identifyProfile({
      ...
      logUtils.presentationStart(
        workflow?.clientId, exchange?.id, profile, userAgent);
      ...
      const {dcApiRequest, authorizationRequest, updatedExchange,
        signingMetadata} = result;

      // In-house DC API profiles (Annex C, Annex D, HAIP, SpruceID) return a
      // ready-to-send wire envelope. Persist exchange and respond.
      if(dcApiRequest) {
        await database.collections.Exchanges.replaceOne(
          {id: exchange.id},
          updatedExchange
        ...
        logUtils.presentationRequestServed({
          ...
          profile, responseMode, wire: 'dcApiRequest', userAgent
        });
        res.set('Content-Type', 'application/json');
        res.json({dcApiRequest});
        return;
      }
  ...
  async authorizationResponseMiddleware(req, res, next) {
    ...
    try {
      ...
      if(callbackResult.success) {
        ...
        // Append allowlisted exchange vars as query params if configured
        // Big vars can break the redirect; keep small or strip to follow
        const {variables} = updatedExchange;
        if(variables && workflow.redirectVariableAllowList) {
          for(const v of workflow.redirectVariableAllowList) {
            if(v in variables) {
              const value = typeof variables[v] === 'string' ?
                variables[v] : JSON.stringify(variables[v]);
              redirect_uri +=
                `&${encodeURIComponent(v)}=${encodeURIComponent(value)}`;
            }
          }
        }
        res.send({
