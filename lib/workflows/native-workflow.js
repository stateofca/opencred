  ...
  async authorizationRequestMiddleware(req, res, next) {
    ...
    try {
      ...
      if(isPost) {
        ...
        if(!accept || !accept.includes('application/oauth-authz-req+jwt')) {
          const errorMessage =
      ...
      const {authorizationRequest, authorizationRequestJwt,
        annexCRequest, updatedExchange, signingMetadata} = result;

      // Handle early return cases (SpruceID handlers that return
      // pre-formatted responses)
      if(annexCRequest || authorizationRequestJwt) {
        // Persist exchange before returning response
        await database.collections.Exchanges.replaceOne(
          {id: exchange.id},
          updatedExchange
        );
        ...
        if(annexCRequest) {
          // Annex C: returns JSON structure with deviceRequest and
          // encryptionInfo
          res.set('Content-Type', 'application/json');
          res.json(annexCRequest);
          return;
        }

        // Annex D: returns JWT string directly
    ...
    try {
      ...
      if(callbackResult.success) {
        const procedurePath = updatedExchange.variables?.procedurePath ??
          (workflow.oidc?.redirectUri ? 'login' : 'verification');
        ...
        const redirect_uri = `${baseUri}/${procedurePath}?exchange_token=${
          token}`;
        res.send({
          redirect_uri,
          exchange: {
