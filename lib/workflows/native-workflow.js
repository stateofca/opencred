        ...
        if(!submitted) {
          ...
          if(vp) {
            ...
            if(!vpResult.verified && vpResult.error) {
              errors.push(vpResult.error.message);
            } else if(!vpResult.verified) {
            ...
            if(!result.verified) {
              ...
              if(result.statusResult?.error) {
                errors.push(result.statusResult.error.message);
              }
            }
          } else {
  ...
  async authorizationRequest(req, res) {
    ...
    try {
      ...
      const authorizationRequest = {
        ...
        client_id: domainToDidWebEncoded(config.server.baseUri),
      ...
      const jwt = await new SignJWT(authorizationRequest)
        .setProtectedHeader({
          alg: key.type,
          kid: `${domainToDidWebEncoded(config.server.baseUri)}#${key.id}`,
          typ: 'JWT'
        })
        .setIssuedAt()
