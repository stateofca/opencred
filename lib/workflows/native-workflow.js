...
/**
 * Strip the pending DC API requests from an exchange about to be persisted.
 *
 * Called in two places, for the same reason: the pending requests are no longer
 * live. On the response side a wallet has answered, so they are spent and their
 * key material is dead weight. On the request side a non-DC-API request has
 * been served on the same exchange, superseding the abandoned DC API offer.
 * Either way, removing them leaves the flat variables shape a non-DC-API
 * exchange has always had — so callbacks, audit, and the success view see no
 * change from multi-profile support, and no later `direct_post` response can be
 * routed against a stale offer.
 *
 * @param {object} updatedExchange - Exchange about to be persisted.
...
export class NativeWorkflowService extends BaseWorkflowService {
  ...
  getProtocols({exchange, workflow} = {}) {
    ...
    // A workflow's own profile wins over the deployment-wide default for the
    // generic `OID4VP` protocol entry; the explicit per-profile entries below
    // are unaffected.
    const oid4vpDefault =
      workflow?.oid4vpProfile ||
  ...
  async authorizationRequestMiddleware(req, res, next) {
    ...
    try {
      ...
      // Step 4: Persist exchange. A non-DC-API request served on an exchange
      // that still carries a spent DC API offer supersedes it: drop the stale
      // pending-request array so the persisted exchange has the flat shape a
      // non-DC-API exchange has always had, and no later `direct_post` response
      // can be misrouted to a DC API handler against dead key material.
      await database.collections.Exchanges.replaceOne(
        {id: exchange.id},
