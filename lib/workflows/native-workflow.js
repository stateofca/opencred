...
export class NativeWorkflowService extends BaseWorkflowService {
  ...
  async formatExchange({exchange, includeQR = false, workflow} = {}) {
    const result = await super.formatExchange({exchange, includeQR, workflow});
    if(!result) {
      return null;
    }
    try {
      const baseUri = (config.server.baseUri || 'https://example.com').replace(
        /\/$/, '');
      ...
      result.continuationUrl = `${baseUri}/verification?exchange_token=${
        encodeURIComponent(token)}`;
    } catch {
      // continuationUrl requires a signing key with id_token purpose;
      // omit when not configured (e.g. in tests)
    }
    return result;
  }
