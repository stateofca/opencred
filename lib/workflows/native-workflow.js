...
import {
  handle18013AnnexDRequest,
  handle18013AnnexDResponse
} from '../spruceid-18013-7.js';
import {
  handleNative18013AnnexDRequest,
  handleNative18013AnnexDResponse
} from './native-18013-7.js';
...
import {getVerifiablePresentationRequest} from './native-vpr.js';
...
export class NativeWorkflowService extends BaseWorkflowService {
  ...
  /**
   * Verify presentation from request body
   * @param {object} options
   * @param {object} options.rp - The relying party config
   * @param {object} options.exchange - The exchange object
   ...
   */
  async verifyPresentationFromRequest({rp, exchange, vpToken}) {
    ...
    if(verified && vc) {
      ...
      // rp.verifiablePresentationRequest exists
      if(rp.verifiablePresentationRequest) {
  ...
  async participateInExchangeMiddleware(req, res, next) {
    ...
    try {
      ...
      } = await this.verifyPresentationFromRequest({
        rp, exchange, vpToken
      });
  ...
  async authorizationRequestMiddleware(req, res, next) {
    ...
    try {
      ...
      // Handle 18013-7-Annex-D profile (DC API over OID4VP)
      if(profile === '18013-7-Annex-D') {
        try {
          // Check if dcApiNamespaceQuery is present in workflow query
          const queryItemWithDcApi =
            rp?.query?.find(item => item.dcApiNamespaceQuery);
          const hasDcApiNamespaceQuery = !!queryItemWithDcApi;

          if(hasDcApiNamespaceQuery) {
            // Use spruceid handler when dcApiNamespaceQuery is present
            const {requests} = await handle18013AnnexDRequest({
              rp,
              exchange,
            ...
            const jwt = requests[0]?.data?.request;
            res.set('Content-Type', 'application/oauth-authz-req+jwt');
            res.send(jwt);
            return;
          } else {
            // Use native handler when dcApiNamespaceQuery is not present
            const {jwt, updatedExchange} = await handleNative18013AnnexDRequest(
              {
                rp,
                exchange,
                requestUrl: req.originalUrl,
      ...
      // Standard OID4VP profile handling
      // Extract responseMode query parameter
      // (defaults to undefined for direct_post)
      ...
      // Handle ephemeral key generation for dc_api.jwt response mode
      ...
      if(responseMode === 'dc_api.jwt') {
        ...
        // Store ephemeral key for response decryption
        exchangeUpdate['variables.ephemeralKeyAgreementPrivateKey'] =
          privateKeyJwk;
      }
  ...
  async authorizationResponseMiddleware(req, res, next) {
    ...
    try {
      // Check if this is a DC API response (18013-7-Annex-D profile)
      // Use configuration-based detection to determine which handler was used,
      // matching the logic from the request handler
      const queryItemWithDcApi =
        rp?.query?.find(item => item.dcApiNamespaceQuery);
      const hasDcApiNamespaceQuery = !!queryItemWithDcApi;
      ...
      if(hasDcApiNamespaceQuery) {
        ...
        try {
          ...
          const {results, finalExchange} = await handle18013AnnexDResponse({
            rp,
            exchange,
            responseUrl,
      ...
      } else if(exchange.variables?.authorizationRequest &&
        exchange.variables?.authorizationRequest?.response_mode.includes(
          'dc_api')) {
        // Use native handler when dcApiNamespaceQuery is not present
        // This handles both 18013-7-Annex-D and dc_api.jwt response modes
        try {
          const responseUrl = `${config.server.baseUri}${req.originalUrl}`;
          const {results, finalExchange} =
      ...
      // Check for dc_api response mode (standard OID4VP with DC API)
      ...
      if(authorizationRequest?.response_mode === 'dc_api' ||
        ...
        try {
          ...
          const vpToken = responseBody.vp_token || responseBody.data?.vp_token;
          const submission = responseBody.presentation_submission ||
          ...
          const {verified, errors, verifiablePresentation} =
            await this.verifySubmission({
              rp, vp_token: vpToken, submission, exchange
            });
          ...
          logUtils.presentationError(
            rp?.clientId, exchange.id, errors.join(', '));
      ...
      // Standard OID4VP response handling (direct_post)
