...
export class NativeWorkflowService extends BaseWorkflowService {
  ...
  async submitPresentation(req, res, next) {
    ...
    try {
      const exchange = await this.getExchange({id: req.params.exchangeId});
      if(exchange) {
  ...
  async authorizationResponse(req, res) {
    ...
    try {
      ...
      if(verified) {
        ...
        const callbackSuccess = await this.sendCallback(
          req.rp.workflow,
          updatedExchange,
          exchange.step
        );
        if(!callbackSuccess) {
          ...
          res.status(400).send({errors: [`Callback failed to send`]});
          return;
        }
