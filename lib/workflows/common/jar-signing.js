 ...
 * @param {string|null} [options.expiresIn='15m'] - JWT `exp` lifetime; pass
 *   `null` to omit the `exp` claim (the working multipaz request has none).
...
export async function signJarJwt({
  ...
  const signer = new SignJWT(authorizationRequest)
