 ...
 * - The deviceRequestInfo field always includes
 *   useCases: [{mandatory: true}] and
 *   documentSets: [[0], [1], …, [N-1]] (one alternative per doc).
 *
...
export function buildItemsRequest({dcqlQuery} = {}) {
  ...
  return creds.map(cred => {
    ...
    for(const claim of cred?.claims ?? []) {
      ...
      // TODO(future): expose per-field intent_to_retain through
      // workflow config. For now default to false (safe: verifier
      // does not retain).
      nameSpaces[ns][field] = false;
    }
...
/**
 * Build the plain `{useCases, documentSets}` object for Annex C
 * `deviceRequestInfo`. Wrapped as tag-24/bstr by the caller via
 * DataItem.
 *
 * Per OpenCred's "any one of these credentials" semantic, the
 * returned `documentSets` is a list of singleton arrays
 * `[[0], [1], …, [N-1]]`, not `[[0, 1, …, N-1]]`.
 *
 * @param {object} options - Options object.
 * @param {number} options.docRequestCount - Number of docRequests
 *   the DeviceRequest will carry.
 * @returns {{useCases: Array, documentSets: Array<Array<number>>}}
 *   Plain DeviceRequestInfo (useCases and documentSets).
 */
