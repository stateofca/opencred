...
export async function verifyJwtSubmission({
  ...
  // Per-workflow override falls back to the global option (default false).
  // Scoped to the holder's presentation-signing key resolved here, not the
  // credential issuer's key verified below.
  const acceptNonCanonicalJwkJcsPub = workflow.acceptNonCanonicalJwkJcsPub ??
    config.opencred.options.acceptNonCanonicalJwkJcsPub;
  ...
  if(!vpResult.verified) {
    ...
    if(vcJwt) {
      // Per-workflow override falls back to the global option (default false).
      const twdiwStatusList2021Enabled = workflow.twdiwStatusList2021Enabled ??
        config.opencred.options.twdiwStatusList2021Enabled;
      const res = await verifyUtils.verifyCredentialJWT(
        vcJwt,
        {checkStatus: verifyUtils.checkStatus, documentLoader,
          twdiwStatusList2021Enabled}
      );
      if(!res.verified) {
 ...
 */
export function _getX5cFromSigningKey(signingKey) {
