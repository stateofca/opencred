...
function _cadmvAndroidClientMetadata({clientName}) {
  // OID4VP 1.0 `vp_formats_supported` (COSE alg integers), matching the working
  // verifier.multipaz.org Google Wallet request. Previously emitted draft-18
  // `vp_formats` with string alg, which a 1.0 wallet does not recognize.
...
function _googleWalletClientMetadata({
  encryptionJwks, clientName, rpMetadataBytes
}) {
  const base = {
    ..._optionalClientName(clientName),
    vp_formats_supported: {
  ...
  // Google-required RP branding metadata (Base64URL-encoded CBOR),
  // emitted verbatim only when configured.
  if(typeof rpMetadataBytes === 'string' && rpMetadataBytes.length > 0) {
    base.gw_rp_metadata_bytes = rpMetadataBytes;
  }
  return Object.freeze(base);
}
 ...
 * @param {string} [options.clientName] - Optional `client_name`.
 * @param {string} [options.rpMetadataBytes] - Base64URL-encoded CBOR
 *   relying-party metadata for google-wallet; emitted as
 *   `gw_rp_metadata_bytes`. Ignored for other profiles.
...
export function buildClientMetadata({
  ...
  switch(profile) {
    ...
    case 'cadmv-android':
      return _cadmvAndroidClientMetadata({clientName});
    case 'OID4VP-HAIP-1.0':
      return _haipClientMetadata({encryptionJwks, clientName});
    case 'google-wallet':
      return _googleWalletClientMetadata({
        encryptionJwks, clientName, rpMetadataBytes
      });
    default:
