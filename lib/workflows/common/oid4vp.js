...
export const getVpFormats = ({profile}) => {
  ...
  // TODO(oid4vp-standard-followup): the standard / draft-18 path still
  // assembles client_metadata inline. Migrate to
  // buildClientMetadata({profile, ...}) from ./client-metadata.js when
  // the standard path's OID4VP 1.0 upgrade is planned. Out of scope for
...
export const getVpFormatsSupported = ({profile}) => {
  ...
  // TODO(oid4vp-standard-followup): see getVpFormats above. Same migration
  // story; deferred with the rest of the standard / draft-18 path.
  const formats = {
