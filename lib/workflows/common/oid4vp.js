...
import {expandTypes} from './type-expansion.js';
...
/**
 * Enrich a single query item with expanded type IRIs.
 *
 ...
 * @param {object} [queryItem.fields] - Fields with type constraints.
 * @param {Array<string>} [queryItem.format] - Credential formats.
 * @returns {Promise<object>} - Enriched query item with typeIris and
 *   fieldTypeIris properties.
 */
async function resolveQueryItem(queryItem) {
  const {
  ...
  const typeIris = (hasContext && hasType) ?
    await expandTypes({types: type, contexts: context}) :
    [];

  const hasFieldTypes = fields?.type &&
    Array.isArray(fields.type) && fields.type.length > 0;

  const fieldTypeIris = (hasContext && hasFieldTypes) ?
    await expandTypes({types: fields.type, contexts: context}) :
    [];

  return {...queryItem, type, context, fields, format, typeIris, fieldTypeIris};
}
...
/**
 * Build a jwt_vc_json or ldp_vc credential query.
 *
 * @param {object} options - Options object.
 * @param {string} options.format - Credential format.
 * @param {Array<string>} options.typeIris - Expanded base type IRIs.
 * @param {Array<string>} options.fieldTypeIris - Expanded field type IRIs.
 * @returns {Promise<object>} - DCQL credential query object.
 */
async function buildVcCredential({format, typeIris, fieldTypeIris}) {
  const claims = [];
  ...
  if(fieldTypeIris.length > 0) {
    ...
    claims.push({path, values: fieldTypeIris});
  }
 ...
 *
 * @param {object} resolved - Query item enriched by resolveQueryItem.
 * @returns {Promise<Array<object>>} - Array of DCQL credential queries.
 */
async function buildCredentialQueries(resolved) {
  const {format: formats, fields, typeIris, fieldTypeIris} = resolved;
  const credentials = [];
  ...
  for(const format of formats) {
    if(format === 'mso_mdoc' && fields) {
      ...
      credentials.push(
        await buildVcCredential({format, typeIris, fieldTypeIris})
      );
    }
  }
...
export const getDcqlQuery = async ({workflow, profile}) => {
  ...
  const resolved = await Promise.all(query.map(resolveQueryItem));
  const credentials = (
