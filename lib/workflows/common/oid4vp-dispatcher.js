...
export async function authorizationResponseForProfile({
  ...
  if(protocol && pending.length > 0 && !pending[0].legacy) {
    ...
    // Legacy flat-slot exchange, or a non-DC-API response such as a
    // `direct_post` form body, where there is nothing to disambiguate.
    [entry] = pending;
    matchedBy = 'legacy';
  }
