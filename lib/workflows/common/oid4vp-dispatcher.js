...
/**
 * Returned when resolving an authorization request for google-wallet profile
 * before Annex D runtime is available.
 */
export class GoogleWalletRuntimeNotImplementedError extends Error {
  constructor(message =
    'google-wallet profile requires Annex D runtime work not yet ' +
    'implemented; see plan 2026-04-27-apple-wallet-annex-c and ' +
    'follow-up plan') {
    super(message);
    this.name = 'GoogleWalletRuntimeNotImplementedError';
    this.statusCode = 501;
    this.errorCode = 'GOOGLE_WALLET_NOT_IMPLEMENTED';
  }
}
...
function _determineHandlerName(profile, workflow) {
  ...
  if(profile === 'google-wallet') {
    return '18013-7-annex-d'; // This may change to the HAIP handler in future.
  }
...
/**
 * Resolves handler name from OID4VP `profile` for tests and diagnostics.
 *
 * @param {string} profile - Profile identifier.
 * @param {object} workflow - The workflow configuration.
 * @returns {string} Handler name (e.g. `18013-7-annex-c`).
 */
export function determineOid4VpHandlerForProfile(profile, workflow) {
  return _determineHandlerName(profile, workflow);
}
 ...
 * @param {boolean} [options.signed] - Annex D only: request a signed JAR
 *   envelope when true (default false).
...
export async function authorizationRequestForProfile({
  ...
  if(profile === 'google-wallet') {
    throw new GoogleWalletRuntimeNotImplementedError();
  }
