...
import {
  ...
  // handleAuthorizationResponse is a passthrough to Annex D's encrypted path.
  // Google Wallet encrypted responses (dc_api.jwt) are structurally identical
  // to Annex D's encrypted path.
  ...
  if(workflow?.dcApiNamespaceQuery) {
    ...
    return 'spruceid';
            ...
            (authorizationRequest.client_id_scheme === 'x509_san_dns' ||
             authorizationRequest.client_id_scheme === 'x509_hash')) {
    // Annex-C, Annex-D, HAIP, or Google Wallet (dcql_query + x509_* scheme)
    // Google Wallet: x509_hash scheme with encrypted response
    if(authorizationRequest.client_id_scheme === 'x509_hash') {
      return 'google-wallet';
    }
