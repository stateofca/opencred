...
function _summarizeEntry({entry, expectedHost}) {
  ...
  try {
    ...
    // The leaf SAN DNS names must include the server host for Apple
    // Wallet reader auth. Google Wallet uses a separate binding
    // (see _checkGoogleMetadataBinding), so this check is Apple-only.
    if(entry.wallet === 'apple-wallet') {
      const {baseUriMatch, warnings} = _checkAppleBaseUriBinding({
        sanDnsNames: summary.sanDnsNames,
        expectedHost
      });
      summary.baseUriMatch = baseUriMatch;
...
/**
 * Check that the leaf cert's SAN DNS names include the server host, as
 * required for Apple Wallet reader auth. Warn-only: returns a structured
 * result with human-readable warning strings; never throws.
 *
 * @param {object} options - Options object.
 * @param {string[]} options.sanDnsNames - Lower-case SAN DNS names.
 * @param {string} options.expectedHost - Lower-case hostname from
 *   `baseUri`.
 * @returns {{baseUriMatch: boolean, warnings: string[]}} Match result
 *   and zero or more warning strings.
 */
function _checkAppleBaseUriBinding({sanDnsNames, expectedHost}) {
  if(sanDnsNames.includes(expectedHost)) {
    return {baseUriMatch: true, warnings: []};
  }
  return {
    baseUriMatch: false,
    warnings: [`leaf SAN DNS names do not include ${expectedHost}`]
  };
}
