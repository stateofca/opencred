...
export function encodeSessionTranscript(options) {
  ...
  // The nested DataItem.fromData pattern is intentional from the
  // spec perspective. @auth0/mdl registers a cbor-x extension on
  // DataItem that emits `#6.24(bstr .cbor value)` on encode. The
  // outer fromData wraps the *inner* DataItem (whose `.buffer`
  // is its plain CBOR), producing the single tag-24 wrapper
  // required by ISO/IEC 18013-5 §9.1.5. Do not collapse to a
  // single fromData — that would emit plain CBOR without the
  // tag-24 wrap and wallets will reject the SessionTranscript.
