 ...
 * Callers that need conformance telemetry when `isLegacyIdentifier` is
 ...
 * @returns {{protocol: string, data: *, isLegacyIdentifier: boolean}}
...
export const unwrapDcApiOid4vpResponse = body => {
  ...
  const isLegacyIdentifier = protocol === DC_API_OID4VP_PROTOCOLS.legacy;
  return {protocol, data: body.data, isLegacyIdentifier};
};
