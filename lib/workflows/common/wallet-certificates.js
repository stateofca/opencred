...
/**
 * Compute the x509_hash client_id for a DER-encoded certificate.
 * Google Wallet uses SHA-256(DER) → base64url-no-padding as the
 * client identifier for JAR-signed requests.
 *
 * @param {Uint8Array} derCert - DER-encoded X.509 certificate.
 * @returns {string} Client ID in `x509_hash:<fingerprint>` format.
 */
export function computeX509HashClientId(derCert) {
  if(!(derCert instanceof Uint8Array)) {
    throw new Error('derCert must be a Uint8Array');
  }
  const hash = crypto.createHash('sha256').update(derCert).digest();
  const fingerprint = Buffer.from(hash)
    .toString('base64url')
    .replace(/=+$/, '');
  return `x509_hash:${fingerprint}`;
}
