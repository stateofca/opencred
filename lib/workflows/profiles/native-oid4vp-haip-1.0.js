...
export async function generateAuthorizationRequest({
  ...
  // Build client_metadata. Per Q7-haip we always populate jwks.keys for
  // HAIP regardless of x5c presence: x5c shares the JAR signing key,
  // while jwks.keys advertises the response-encryption key — different
  // keys, different purposes.
  ...
  // HAIP per OID4VP 1.0 §5 mandates signed requests; the unsigned flow
  ...
  const authorizationRequest = {
    ...
    // Defensive: OID4VP 1.0 §B.2.6 does not require `state` for the DC
    // API flow, but earlier wallets may rely on it for session
    // correlation. We still emit and verify it; this can be retired
    // once telemetry confirms no live wallet depends on it.
  ...
  // Calculate updated exchange state
  ...
  // Preserve middleware-era wire kid behavior: when signingMetadata is
