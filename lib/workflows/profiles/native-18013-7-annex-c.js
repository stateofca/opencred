...
export async function generateAuthorizationRequest({
  ...
  // Build the same plain DeviceRequestInfo that `buildDeviceRequest`
  // will emit, so the bytes we sign over for readerAuthAll match the
  // bytes that reach Apple Wallet on the wire. buildDeviceRequestInfo
  // is pure, so calling it twice (once here, once inside
  // buildDeviceRequest) yields byte-identical output.
  const deviceRequestInfo = buildDeviceRequestInfo({
    docRequestCount: itemsRequestList.length
  });
...
export async function handleAuthorizationResponse({
  ...
  if(!base64EncryptedResponse) {
    throw new Error('Response not found in response body');
  }
  if(typeof base64EncryptedResponse !== 'string') {
