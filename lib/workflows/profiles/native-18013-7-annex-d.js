...
export async function generateAuthorizationRequest({
  ...
  const authorizationRequest = {
    ...
    // Defensive: OID4VP 1.0 §B.2.6 does not require `state` for the DC API
    // flow, but earlier wallets may rely on it for session correlation. We
    // still emit and verify it; this can be retired once telemetry confirms
    // no live wallet depends on it.
    // TODO(retire-state): remove once telemetry confirms no dependency.
...
export async function handleAuthorizationResponse({
  ...
  // Defensive state validation; see TODO(retire-state) in
  // generateAuthorizationRequest for context.
  ...
  if(responseMode === 'dc_api.jwt') {
    ...
    try {
      ...
      if(typeof vpToken === 'string') {
        ...
        vpTokenString = _normalizeVpTokenValue(vpToken[credentialId]);
        if(!vpTokenString) {
    ...
    if(typeof vpTokenString === 'string' && _isEncryptedJWT(vpTokenString)) {
      ...
      try {
        ...
        if(typeof decryptedVpToken === 'string') {
          ...
          // Use first credential ID (credentialIds already validated above)
          const credentialId = credentialIds[0];
          vpTokenString = _normalizeVpTokenValue(
...
/**
 * Normalize a vp_token value that may be a string or an array.
 * OID4VP 1.0 with DCQL may return credential values as arrays.
 *
 * @param {string|Array} value - Value from vp_token for a credential ID.
 * @returns {string} The token string.
 */
function _normalizeVpTokenValue(value) {
