...
/**
 * @deprecated Use wallet-specific handlers instead. See:
 *   - cadmv-android (native-cadmv-android.js)
 *   - cadmv-ios (native-cadmv-ios.js)
 *   - google-wallet (native-google-wallet.js)
 *   - apple-wallet (native-18013-7-annex-c.js).
 *
 * "Annex D" was a concept for a generic profile that never converged on real
 * requirements. Every wallet interprets the concapt differently.
 * Wallet-specific handlers encode the exact wire format each wallet expects.
 */
 ...
 * @param {boolean} [options.signed=false] - When true, return a signed JAR
 *   envelope; when false, unsigned Annex D wire object.
...
export async function generateAuthorizationRequest({
  ...
  // Spec-clean (match working verifier.multipaz.org request): drop
  // credential_sets — OID4VP 1.0 makes it optional and Google Wallet's working
  // request omits it. credentials[] is still present and authoritative.
  const dcql_query = await _buildDcqlQueryForMdoc({
    workflow, exchange, profile});
  if(dcql_query?.credential_sets) {
    delete dcql_query.credential_sets;
  }

  const client_metadata = buildClientMetadata({profile: '18013-7-Annex-D'});

  // Get x5c certificate chain (excluding trust anchor per HAIP)
  const x5c = _getX5cFromSigningKey(signingKey);

  // Build authorization request.
  // Spec-clean to match the working verifier.multipaz.org request: omit
  // client_id_scheme (OID4VP 1.0 carries the scheme as the client_id prefix)
  // and omit state (optional for the DC API flow; the working request omits
  // it). The response handler still validates state defensively if a wallet
  // happens to echo one.
  ...
  let dcApiRequest;
  if(signed === true) {
    // Preserve kid behavior: when signingMetadata is
    // provided, signJarJwt uses signingMetadata.kid (`hostname#keyId`).
    // The fallback kid (`did:web:hostname#keyId`) is only used when
    // signingMetadata is omitted.
    const signedJwt = await signJarJwt({
      authorizationRequest,
      signingKey,
      kid: `${domainToDidWeb(serverBaseUri)}#${signingKey.id}`,
      signingMetadata
    });
    dcApiRequest = buildAnnexDDcApiRequest({signed: true, signedJwt});
  } else {
    dcApiRequest = buildAnnexDDcApiRequest({
      signed: false,
      authorizationRequest
    });
  }
...
export async function handleAuthorizationResponse({
  ...
  // Handle DC API OID4VP protocol envelope (legacy + OID4VP 1.0 ids)
  let actualResponseBody = responseBody;
  if(
    responseBody &&
    typeof responseBody.protocol === 'string' &&
    DC_API_OID4VP_ACCEPTED_PROTOCOLS.includes(responseBody.protocol) &&
    ...
    if(isLegacyProtocol) {
      logUtils.legacyProtocolIdentifier({
        ...
        metadata: {observedProtocol: protocol, source: 'dc_api_response'}
      });
    }
    actualResponseBody = data;
  }

  if(actualResponseBody?.error) {
    throw new Error(
      `Wallet returned OID4VP error response: ${actualResponseBody.error}`
    );
  }

  // Defensive state validation. The request no longer emits `state` (spec-clean
  // to match the working Google Wallet request), so this only fires if a wallet
  // echoes a state and the request happens to carry one.
  ...
  if(responseMode === 'dc_api.jwt') {
    ...
    try {
      ...
      if(typeof vpToken === 'string') {
        ...
        vpTokenString = normalizeVpTokenValue(vpToken[credentialId]);
        if(!vpTokenString) {
    ...
    if(typeof vpTokenString === 'string' && _isEncryptedJWT(vpTokenString)) {
      ...
      try {
        ...
        if(typeof decryptedVpToken === 'string') {
          ...
          // Use first credential ID (credentialIds already validated above)
          const credentialId = credentialIds[0];
          vpTokenString = normalizeVpTokenValue(
            decryptedVpToken[credentialId]);
          if(!vpTokenString) {
  ...
  if(!encodedSessionTranscript) {
    ...
    encodedSessionTranscript = encodeSessionTranscript({
      responseMode,
      origin,
      clientId,
      nonce,
