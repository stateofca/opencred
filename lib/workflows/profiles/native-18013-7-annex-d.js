 ...
 * @param {string} options.origin - Origin (server base URI).
 * @param {string} options.nonce - Nonce from authorization request.
 * @returns {Uint8Array} Encoded session transcript.
 */
function _createSessionTranscriptAnnexD({origin, nonce}) {
  // OID4VP 1.0 Appendix B.2.6.2:
  // OpenID4VPDCAPIHandoverInfo = [origin, nonce, jwkThumbprint]
  // For unsigned `dc_api` flow, the third item is null
  const handoverInfo = [origin, nonce, null];
...
export async function generateAuthorizationRequest({
  ...
  // For mso_mdoc, OID4VP 1.0 defines `issuerauth_alg_values` &
  // `deviceauth_alg_values`
  // TODO(oid4vp-1.0-phase-6): replace with buildClientMetadata({profile, ...})
  // from ../common/client-metadata.js
