...
export async function generateAuthorizationRequest({
  ...
  // This returns {requests: [{protocol: "openid4vp",
  ...
  if(isAnnexC) {
    ...
    // Extract Annex D request (protocol: "openid4vp") - default behavior
    const annexDRequest = requests?.find(req =>
      req.protocol === 'openid4vp'
    );
    if(!annexDRequest || !annexDRequest.data?.request) {
      throw new Error(
        'No authorization request JWT returned from DC API. ' +
        'Expected requests array to contain an object with protocol ' +
        '"openid4vp" and data.request property.'
      );
    }
