...
async function createDcApiInstance(exchange, responseUrl) {
  ...
  // Build certificate chain: signing cert (full chain) + caStore IACA roots.
  // The WASM module uses all certs for both request signing identity (x5c)
  // and trust anchor registry (mDL issuer validation).
  //
  // TODO: There are two problems with this approach.
  // 1. The WASM module uses all certs for both request signing identity (x5c)
  //    and trust anchor registry (mDL issuer validation).
  // 2. The WASM module uses the wrong trust purpose for the trust anchors.
  //    It should use TrustPurpose::Iaca for the trust anchors.
  const certChainPem = buildCertChainPem({
...
/**
 * Build a concatenated PEM certificate chain for the WASM DC API module.
 *
 * The WASM module uses the certificate chain for two purposes:
 * 1. Request signing identity (x5c in JWT header / COSE ReaderAuth).
 * 2. Trust anchor registry for mDL issuer validation (IACA roots).
 *
 * @param {object} options - Options object.
 * @param {string} options.signingCertPem - The signing key's certificatePem
 *   (may contain a full chain: leaf + intermediates).
 * @param {Array<string>|Array<{pem: string}>} [options.caStoreCerts] - Array of
 *   caStore certificates. Can be an array of PEM strings (runtime format) or an
 *   array of objects with a `pem` property (documented format).
 * @returns {string} Concatenated PEM string containing signing cert chain
 *   followed by caStore certificates.
 */
function buildCertChainPem({signingCertPem, caStoreCerts = []}) {
  // Start with the full signing cert chain (no truncation)
  const parts = [normalizePem(signingCertPem)];

  // Append each caStore certificate (IACA roots for mDL validation)
  for(const cert of caStoreCerts) {
    if(typeof cert === 'string') {
      // caStore is an array of PEM strings (runtime format)
      parts.push(normalizePem(cert));
    } else if(cert?.pem) {
      // caStore is an array of {pem: string} objects (documented format)
      parts.push(normalizePem(cert.pem));
    }
  }
...
export {buildCertChainPem, createSessionStores};
