...
export async function generateAuthorizationRequest({
  ...
  if(isAnnexC) {
    ...
    // Store Annex C data in exchange variables for response handling
    ...
    // Update exchange with authorization request (decoded) and JWT string
...
export async function handleAuthorizationResponse({
  ...
  const results = await dcApi.submit_response(
    ...
    sessionSecret,
    {
      protocol: SPRUCEID_WASM_PROTOCOL,
      data: wasmData
    }
  );
