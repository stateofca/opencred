...
export async function generateAuthorizationRequest({
  ...
  // The initiate_request returns {requests: [{protocol: "openid4vp",
  //   data: {request: "jwtstring"}}]}
  const firstRequest = requests?.[0];
  const jwtString = firstRequest?.data?.request;
  if(!jwtString) {
    throw new Error(
      'No authorization request JWT returned from DC API. ' +
      'Expected requests to be an array of objects with data.request property.'
    );
  }
