...
export async function generateAuthorizationRequest({
  ...
  // Spec-clean to match the working verifier.multipaz.org Google Wallet
  // request: drop credential_sets (OID4VP 1.0 makes it optional; credentials[]
  ...
  // Spec-clean: omit client_id_scheme — OID4VP 1.0 carries the scheme as the
  // client_id prefix (x509_san_dns:...). state is intentionally absent.
  ...
  const signedJwt = await signJarJwt({
    ...
    signingMetadata,
    // Match a proven test vector which did not have an expiration.
    // TODO: This could be reversed later after a working pass.
    expiresIn: null
  });
