...
export async function generateAuthorizationRequest({
  ...
  const [
    ...
    getDcqlQuery({
      workflow, profile}),
    // TODO(oid4vp-standard-followup): migrate to buildClientMetadata({
    // profile, ...}) from ../common/client-metadata.js when the standard
    // / draft-18 path's OID4VP 1.0 upgrade is planned. Out of scope for
    // the 18013-7 / HAIP DC API upgrade.
    getClientMetadata({profile})
  ]);

  // client_id and client_id_scheme by profile. OID4VP 1.0: not used
  // client_id_scheme, use decentralized_identifier: prefix in client_id.
  let clientId;
  let signingMetadata;
  ...
  if(clientIdScheme === 'x509_san_dns') {
    ...
    // DID scheme: OID4VP 1.0 uses prefix; draft18 uses bare DID
    clientId = clientIdForProfile({profile, domain: serverBaseUri});
  }
  ...
  const authorizationRequest = {
    ...
    client_id: clientId,
    ...clientIdSchemeForProfile({profile, clientIdScheme}),
