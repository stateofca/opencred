...
function _determineHandlerName(profile, workflow) {
  // Handle 18013-7-Annex-D profile refinement
  // Check if dcApiNamespaceQuery is present at workflow level
  if(profile === '18013-7-Annex-D' && workflow?.dcApiNamespaceQuery) {
    return 'spruceid';
  }
 ...
 * @returns {string} Handler name
 */
function _determineHandlerFromRequest(authorizationRequest, workflow) {
  ...
  if(!!workflow?.dcApiNamespaceQuery) {
    ...
    // Annex-D or HAIP handler (uses x509_san_dns and dcql_query)
