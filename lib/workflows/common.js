...
import {resolveContextRedirectUri} from '../redirect-uri.js';
...
export async function buildNewExchangeContextData(
  ...
  const interactEnabled =
    workflow.interactEnabled !== undefined ?
      workflow.interactEnabled !== false :
      config.opencred.options.interactEnabled !== false;

  const connectionPickerEnabled =
    workflow.connectionPickerEnabled !== undefined ?
      workflow.connectionPickerEnabled !== false :
      config.opencred.options.connectionPickerEnabled !== false;
  ...
  const options = {
    ...
    // Resolve the workflow's own OID4VP profile over the deployment default so
    // the frontend, which reads options.OID4VPdefault to pick the default
    // profile, honours a per-workflow override without a separate field.
    OID4VPdefault: workflow.oid4vpProfile ??
      config.opencred.options.OID4VPdefault
  };

  const contextRedirectUri = workflow.oidc ?
    resolveContextRedirectUri({workflow, exchange}) :
    undefined;
  ...
  return {
    ...
    workflow: {
      ...
      interactEnabled,
      connectionPickerEnabled,
      // Drives the DC API launch buttons. Non-sensitive: button ids, labels,
      // and profile names. Absent when unconfigured, in which case the UI
      // derives one button per enabled compatible wallet.
      ...(workflow.dcApiButtons ? {dcApiButtons: workflow.dcApiButtons} : {}),
      // Selects and orders the connection options the browser assembles from
      // this exchange. Non-sensitive: interaction methods, profile names, and
      // presentation labels. Absent when unconfigured, in which case the UI
      // shows the derived options in their derived order.
      ...(workflow.connectionOptions ?
        {connectionOptions: workflow.connectionOptions} : {}),
      // The wallets the install invitation promotes. Non-sensitive: wallet
      // identifiers. Absent when unconfigured, in which case the invitation
      // promotes every enabled wallet with a storefront, exactly as before.
      ...(workflow.promotedWallets ?
        {promotedWallets: workflow.promotedWallets} : {}),
      ...(contextRedirectUri ?
        {oidc: {redirectUri: contextRedirectUri}} : {})
    },
    options,
    exchangeData
  };
...
/**
 * Resolves configured success-view field paths against exchange variables.
 * Present-only: entries whose path does not resolve to a defined value are
 * skipped. Resolve against the exchange AS SENT (post-scrub) so a misconfigured
 * path cannot bypass partial-scope scrubbing. `label`/`labelKey` are passed
 * through untouched (no server-side i18n); the frontend chooses which to show.
 *
 * @param {object} options - Options.
 * @param {object} options.workflow - Workflow config (with successViewFields).
 * @param {object} options.exchange - Exchange (with variables) being returned.
 * @returns {Array<{label?: string, labelKey?: string, value: *}>} Resolved
 *   rows.
 */
export function resolveSuccessViewFields({workflow, exchange}) {
  const fields = workflow?.successViewFields ?? [];
  const variables = exchange?.variables ?? {};
  const rows = [];
  for(const {path, label, labelKey} of fields) {
    const jsonPath = path.startsWith('$') ? path : `$.${path}`;
    const values = JSONPath({path: jsonPath, json: variables});
...
export async function handleVerifiedPresentation({
  ...
  const updatedExchange = {
    ...
    oidc: {
      ...
      ...(exchange.oidc?.redirectUri ?
        {redirectUri: exchange.oidc.redirectUri} : {})
    },
    variables: {
      ...(exchange.variables ?? {}),
      results: {
 ...
 * @remarks Uses the first signing key whose purpose includes `access_token`
 *   (exchange-capability JWT; not the OIDC `id_token` purpose).
 */
export async function buildExchangeResultToken({
  ...
  const signingKey = config.opencred.signingKeys
    .find(k => k.purpose?.includes('access_token'));
  if(!signingKey) {
    throw new Error('No signing key with access_token purpose found');
  }
