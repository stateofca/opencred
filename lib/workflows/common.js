...
export const newExchangeContext = async (req, res) => {
  ...
  // Build options object with workflow-specific overrides
  const options = {
    ...config.opencred.options,
    // Override dcApiEnabled if workflow has a specific setting
    dcApiEnabled: workflow.dcApiEnabled !== undefined ?
      (isDcApiAvailable(config.opencred) && workflow.dcApiEnabled === true) :
      dcApiEnabled,
    // Override wallets if workflow has a specific setting
    wallets: workflow.wallets !== undefined ?
      workflow.wallets :
      config.opencred.options.wallets
  };
