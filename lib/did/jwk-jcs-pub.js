 ...
 * driver.
 *
 * When `acceptNonCanonical` is set, the malleability gate is relaxed: an
 * embedded JWK whose members are in a non-canonical order (or that carries
 * extra members) is decoded from whatever order it uses rather than rejected.
 * The coordinates themselves are still required to be valid canonical P-256
 * coordinates and the key must still be a usable P-256 key — only the
 * one-key-one-DID commitment is waived. The default stays strict.
 *
 * @param {object} options - Options.
 * @param {string} options.did - A `did:key` identifier (optionally with a
 *   `#fragment`).
 * @param {boolean} [options.acceptNonCanonical=false] - Accept a valid P-256
 *   key whose embedded JWK is not in canonical JCS member order.
 ...
 *   jwk_jcs-pub key.
 */
export const decodeJwkJcsPubDidKey = async (
  {did, acceptNonCanonical = false} = {}) => {
  ...
  // coordinates, all of which fail to reproduce the exact input DID. When
  // acceptNonCanonical is set the gate is waived — the coordinates were already
  // validated above, so what remains is only the one-key-one-DID commitment,
  // which a lenient workflow opts out of to interoperate with a wallet that
  // serializes JWK members in a non-canonical order.
  if(!acceptNonCanonical &&
    canonicalJwkJcsPubDid({jwk: canonicalJwk}) !== authority) {
    throw new Error('non-canonical jwk_jcs-pub did:key rejected');
  }
 ...
 */
export const resolveJwkJcsPubDidKey = async (
  {id, acceptNonCanonical = false} = {}) => {
  const decoded = typeof id === 'string' ?
    await decodeJwkJcsPubDidKey({did: id, acceptNonCanonical}) : null;
  if(!decoded) {
    return null;
  }
