...
import {jwtFromExchange} from '../common/oidc.js';
import {logger} from './logger.js';

/**
 * Middleware for OIDC that validates requests prior to more in-depth
 * processing. Validates redirect_uri and scope parameters.
 *
 * @param {object} req - Express request object.
 * @param {object} res - Express response object.
 * @param {Function} next - Express next middleware function.
 */
export const validateOidcLoginMiddleware = async (req, res, next) => {
  ...
  if(!req.query.redirect_uri) {
    ...
    return;
  } else if(req.workflow?.oidc?.redirectUri != req.query.redirect_uri) {
    res.status(400).send({
      error: 'invalid_grant',
...
export const exchangeCodeForToken = async (req, res) => {
  ...
  if(!exchange) {
    ...
    return;
  } else if(exchange.workflowId !== workflow.clientId) {
    res.status(400).send({
      error: 'invalid_grant',
  ...
  try {
    const jwt_string = await jwtFromExchange(exchange, workflow);
    const token = {
      access_token: 'NONE',
      token_type: 'Bearer',
      expires_in: 3600,
...
export const openIdConfiguration = async (req, res) => {
  ...
  const info = {
    ...
    service_documentation: 'https://github.com/stateofca/opencred',
