...
export const openIdConfiguration = async (req, res) => {
  ...
  const info = {
    issuer: config.opencred.domain,
    authorization_endpoint: config.opencred.domain + '/login',
    token_endpoint: config.opencred.domain + '/token',
    jwks_uri: config.opencred.domain + '/.well-known/jwks.json',
