...
import {
  formatExchangeExpires, resolveExchangeExpires
...
export const getExchangeStatus = async (req, res) => {
  ...
  if(!sanitized.expires) {
    const resolved = resolveExchangeExpires({exchange: sanitized});
    if(resolved) {
      sanitized.expires = formatExchangeExpires(resolved);
    }
  }
  const toSend = req.exchangeScope === 'partial' ?
    scrubExchangeForPartialScope(sanitized) : sanitized;
  toSend.successViewFields = resolveSuccessViewFields({
    workflow: req.workflow, exchange: toSend
  });
...
export const getConfig = (req, res) => {
  const workflow = findWorkflow(
    bedrock.config.opencred.workflows, req.query.client_id
  );
  const {
    defaultLanguage, translations: translationsDraft, options, defaultBrand,
    customTranslateScript, audit, fontUrls,
